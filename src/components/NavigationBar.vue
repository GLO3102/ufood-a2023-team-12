...
<template>
<div>
<nav class="navbar navbar-expand-lg fixed-top navbar-dark px-5" style="background-color: #0A0908;">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="col navbar-nav mr-auto">
      <li class="nav-item">
        <router-link class="nav-link alt-font" to="/">Home</router-link>
      </li>
    </ul>
    <form v-if="innerWidth >= navCollapseValue" class="col form-inline">
      <div class="input-group">
        <input class="form-control mr-sm-2 nav_searchbar alt-font" type="search" placeholder="Search" aria-label="Search">
      </div>
    </form>
    <ul class="col justify-content-end navbar-nav">
      <li v-if="loggedIn" class="nav-item" id="username">
        <router-link class="nav-link text-warning alt-font" to="/user">{{txt_user}}</router-link>
      </li>
      <li v-if="!loggedIn" class="nav-item">
        <router-link class="nav-link alt-font" to="/login">Login</router-link>
      </li>
      <li v-if="loggedIn" class="nav-item" id="btn_logout">
        <router-link class="nav-link alt-font" to="/" @click="logout">Logout</router-link>
      </li>
    </ul>
  </div>
</nav>
